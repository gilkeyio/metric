...
from metric.tokenizer import tokenize, Token, IntegerToken, IdentifierToken, FloatToken, TokenizerError
...
class TestTokenizer(unittest.TestCase):
    ...
    def test_tokenize_multiple_newlines(self):
        ...
        expected = [Token.LET, IdentifierToken("x"), Token.INTEGER_TYPE, Token.EQUALS, IntegerToken(5), Token.STATEMENT_SEPARATOR, 
                   Token.PRINT, IdentifierToken("x")]
        self.assertEqual(result, expected)
    
    def test_tokenize_single_digit(self):
        result = tokenize("5")
        self.assertEqual(result, [IntegerToken(5)])
    
    def test_tokenize_multiple_digits(self):
        result = tokenize("123")
    ...
    def test_tokenize_logical_and_expression(self):
        result = tokenize("true and false")
        self.assertEqual(result, [Token.TRUE, Token.AND, Token.FALSE])
    
    def test_tokenize_logical_or_expression(self):
        result = tokenize("true or false")
        self.assertEqual(result, [Token.TRUE, Token.OR, Token.FALSE])
    
    def test_tokenize_logical_not_expression(self):
        result = tokenize("not true")
        self.assertEqual(result, [Token.NOT, Token.TRUE])
    
    def test_tokenize_complex_logical_expression(self):
        result = tokenize("x and y or z")
        expected = [IdentifierToken("x"), Token.AND, IdentifierToken("y"), Token.OR, IdentifierToken("z")]
        self.assertEqual(result, expected)
    
    def test_tokenize_complex_logical_with_not_expression(self):
        result = tokenize("not x and y")
        expected = [Token.NOT, IdentifierToken("x"), Token.AND, IdentifierToken("y")]
        self.assertEqual(result, expected)
    
    def test_tokenize_plus_operator(self):
        result = tokenize("+")
        self.assertEqual(result, [Token.PLUS])
    
    def test_tokenize_minus_operator(self):
        result = tokenize("-")
        self.assertEqual(result, [Token.MINUS])
    
    def test_tokenize_multiply_operator(self):
        result = tokenize("*")
        self.assertEqual(result, [Token.MULTIPLY])
    
    def test_tokenize_divide_operator(self):
        result = tokenize("/")
        self.assertEqual(result, [Token.DIVIDE])
    
    def test_tokenize_left_parenthesis(self):
        result = tokenize("(")
        self.assertEqual(result, [Token.LEFT_PARENTHESIS])
    
    def test_tokenize_right_parenthesis(self):
        result = tokenize(")")
    ...
    def test_tokenize_simple_expression_with_spaces(self):
        result = tokenize("1 + 2")
    ...
    def test_tokenize_complex_expression(self):
        ...
        expected = [Token.LEFT_PARENTHESIS, IntegerToken(1), Token.PLUS, IntegerToken(2), 
                   Token.RIGHT_PARENTHESIS, Token.MULTIPLY, IntegerToken(3)]
        self.assertEqual(result, expected)
    
    def test_tokenize_multiple_statements_with_newlines(self):
        result = tokenize("let x integer = 5\nprint x")
        expected = [Token.LET, IdentifierToken("x"), Token.INTEGER_TYPE, Token.EQUALS, IntegerToken(5), 
                   Token.STATEMENT_SEPARATOR, Token.PRINT, IdentifierToken("x")]
        self.assertEqual(result, expected)
    
    def test_tokenize_identifier_followed_by_end_of_input(self):
        result = tokenize("x")
        self.assertEqual(result, [IdentifierToken("x")])
    
    def test_tokenize_number_followed_by_end_of_input(self):
        result = tokenize("42")
        self.assertEqual(result, [IntegerToken(42)])
    
    def test_tokenize_identifier_followed_by_parenthesis(self):
        result = tokenize("x(")
        self.assertEqual(result, [IdentifierToken("x"), Token.LEFT_PARENTHESIS])
    
    def test_tokenize_number_followed_by_parenthesis(self):
        result = tokenize("42)")
        self.assertEqual(result, [IntegerToken(42), Token.RIGHT_PARENTHESIS])

    # Boolean literal tests
    def test_tokenize_true_literal(self):
        result = tokenize("true")
        self.assertEqual(result, [Token.TRUE])
    
    def test_tokenize_false_literal(self):
    ...
    def test_tokenize_boolean_expression(self):
        result = tokenize("let result = x > y")
        expected = [Token.LET, IdentifierToken("result"), Token.EQUALS, IdentifierToken("x"), Token.GREATER_THAN, IdentifierToken("y")]
        self.assertEqual(result, expected)
    ...
    def test_tokenize_simple_if_block(self):
        code = code_block("""if x > 5
    print x""")
        result = tokenize(code)
        expected = [Token.IF, IdentifierToken("x"), Token.GREATER_THAN, IntegerToken(5), Token.STATEMENT_SEPARATOR, Token.INDENT, Token.PRINT, IdentifierToken("x"), Token.DEDENT]
        self.assertEqual(result, expected)
    
    def test_tokenize_if_with_multiple_indented_statements(self):
    ...
    def test_tokenize_if_with_dedent(self):
        code = code_block("""if true
    print x
print y""")
        result = tokenize(code)
    ...
    def test_tokenize_simple_while_block(self):
        code = code_block("""while x > 0
    print x""")
        result = tokenize(code)
        expected = [Token.WHILE, IdentifierToken("x"), Token.GREATER_THAN, IntegerToken(0), Token.STATEMENT_SEPARATOR, Token.INDENT, Token.PRINT, IdentifierToken("x"), Token.DEDENT]
        self.assertEqual(result, expected)
    
    def test_tokenize_while_with_set(self):
        code = code_block("""while true
    set x = x + 1""")
        result = tokenize(code)
        expected = [Token.WHILE, Token.TRUE, Token.STATEMENT_SEPARATOR, Token.INDENT, Token.SET, IdentifierToken("x"), Token.EQUALS, IdentifierToken("x"), Token.PLUS, IntegerToken(1), Token.DEDENT]
        self.assertEqual(result, expected)
    
    def test_tokenize_set_statement(self):
        result = tokenize("set counter = counter + 1")
        expected = [Token.SET, IdentifierToken("counter"), Token.EQUALS, IdentifierToken("counter"), Token.PLUS, IntegerToken(1)]
        self.assertEqual(result, expected)
    
    # Type annotation tests
    def test_tokenize_integer_type(self):
        result = tokenize("integer")
        self.assertEqual(result, [Token.INTEGER_TYPE])
    
    def test_tokenize_boolean_type(self):
        result = tokenize("boolean")
        self.assertEqual(result, [Token.BOOLEAN_TYPE])
    
    def test_tokenize_typed_let_statement(self):
        result = tokenize("let x integer = 5")
        expected = [Token.LET, IdentifierToken("x"), Token.INTEGER_TYPE, Token.EQUALS, IntegerToken(5)]
        self.assertEqual(result, expected)
    
    def test_tokenize_typed_boolean_let(self):
        result = tokenize("let flag boolean = true")
        expected = [Token.LET, IdentifierToken("flag"), Token.BOOLEAN_TYPE, Token.EQUALS, Token.TRUE]
        self.assertEqual(result, expected)
    
    def test_tokenize_complex_typed_program(self):
...
print y""")
    ...
    def test_tokenize_modulus_operator(self):
        result = tokenize("5 % 3")
        self.assertEqual(result, [IntegerToken(5), Token.MODULUS, IntegerToken(3)])
    
    def test_tokenize_modulus_expression(self):
        result = tokenize("let remainder integer = x % y")
        expected = [Token.LET, IdentifierToken("remainder"), Token.INTEGER_TYPE, Token.EQUALS, IdentifierToken("x"), Token.MODULUS, IdentifierToken("y")]
        self.assertEqual(result, expected)
    ...
    def test_tokenize_float_zero(self):
        result = tokenize("0.0")
        self.assertEqual(result, [FloatToken(0.0)])
    
    def test_tokenize_float_with_trailing_zeros(self):
        result = tokenize("1.500")
        self.assertEqual(result, [FloatToken(1.5)])
    
    def test_tokenize_float_expression(self):
        result = tokenize("3.14 + 2.5")
        expected = [FloatToken(3.14), Token.PLUS, FloatToken(2.5)]
        self.assertEqual(result, expected)
    
    def test_tokenize_float_type_annotation(self):
        result = tokenize("float")
        self.assertEqual(result, [Token.FLOAT_TYPE])
    
    def test_tokenize_typed_float_let(self):
        result = tokenize("let pi float = 3.14")
        expected = [Token.LET, IdentifierToken("pi"), Token.FLOAT_TYPE, Token.EQUALS, FloatToken(3.14)]
        self.assertEqual(result, expected)
    
    def test_tokenize_mixed_int_float_expression(self):
        result = tokenize("let result float = 5 + 3.14")
        expected = [Token.LET, IdentifierToken("result"), Token.FLOAT_TYPE, Token.EQUALS, IntegerToken(5), Token.PLUS, FloatToken(3.14)]
        self.assertEqual(result, expected)
    
    def test_tokenize_fails_with_incomplete_float(self):
    ...
    def test_tokenize_fails_with_multiple_decimal_points(self):
        with self.assertRaises(TokenizerError) as cm:
            tokenize("3.14.5")
        self.assertEqual(str(cm.exception), "Unexpected character: . at line 1")
    
    # Comment tokenization tests
    def test_tokenize_standalone_comment(self):
        result = tokenize("# This is a comment")
        expected = [Token.COMMENT]
        self.assertEqual(result, expected)
    
    def test_tokenize_comment_after_statement(self):
        result = tokenize("let x integer = 5 # This is a comment")
        expected = [Token.LET, IdentifierToken("x"), Token.INTEGER_TYPE, Token.EQUALS, IntegerToken(5), Token.COMMENT]
        self.assertEqual(result, expected)
    
    def test_tokenize_comment_with_proper_indentation(self):
        result = tokenize("    # Indented comment")
        expected = [Token.INDENT, Token.COMMENT, Token.DEDENT]
        self.assertEqual(result, expected)
    
    def test_tokenize_mixed_statements_and_comments(self):
...
print x # Print the value""")
    ...
    def test_tokenize_comment_with_special_characters(self):
        result = tokenize("# Comment with !@#$%^&*()=+")
        expected = [Token.COMMENT]
        self.assertEqual(result, expected)
    
    def test_tokenize_comment_only_program(self):
...
# Third comment""")
    ...
    def test_tokenize_comment_between_statements(self):
...
print x""")
    ...
    def test_tokenize_comment_with_indented_blocks(self):
        code = code_block("""if true
    # Comment in if block
    print x
# Comment after if block""")
    ...
    def test_tokenize_return_statement(self):
    ...
    def test_tokenize_list_type_declaration(self):
    ...
    def test_tokenize_empty_list_literal(self):
        """Test empty list literal tokenization."""
        code = "[]"
        result = tokenize(code)
        expected = [Token.LEFT_BRACKET, Token.RIGHT_BRACKET]
        self.assertEqual(result, expected)
    
    def test_tokenize_list_with_boolean_elements(self):
    ...
    def test_tokenize_list_with_float_elements(self):
    ...
    def test_tokenize_repeat_function_call(self):
    ...
    def test_tokenize_len_function_call(self):
    ...
    def test_tokenize_list_access(self):
    ...
    def test_tokenize_nested_list_access(self):
    ...
    def test_tokenize_complex_list_declaration(self):
    ...
    def test_tokenize_list_in_function_parameter(self):
    ...
    def test_tokenize_list_return_type(self):
    ...
    def test_tokenize_list_keywords_are_recognized(self):
        """Test that list keywords are properly tokenized as keywords, not identifiers."""
        result = tokenize("let list = 5")
        expected = [Token.LET, Token.LIST, Token.EQUALS, IntegerToken(5)]
        self.assertEqual(result, expected)
        
        result = tokenize("repeat(1, 5)")
        expected = [Token.REPEAT, Token.LEFT_PARENTHESIS, IntegerToken(1), Token.COMMA, IntegerToken(5), Token.RIGHT_PARENTHESIS]
        self.assertEqual(result, expected)
    ...
    def test_tokenize_negative_zero(self):
        """Test tokenizing negative zero."""
        result = tokenize("-0")
        self.assertEqual(result, [IntegerToken(0)])
    
    def test_tokenize_negative_large_integer(self):
        """Test tokenizing negative large integers."""
        result = tokenize("-999999")
        self.assertEqual(result, [IntegerToken(-999999)])
    
    def test_tokenize_negative_integer_in_expression(self):
        """Test tokenizing negative integers in expressions."""
        result = tokenize("let x integer = -42")
        expected = [Token.LET, IdentifierToken("x"), Token.INTEGER_TYPE, Token.EQUALS, IntegerToken(-42)]
        self.assertEqual(result, expected)
    
    def test_tokenize_negative_integer_arithmetic(self):
        """Test tokenizing negative integers in arithmetic expressions."""
        result = tokenize("-5 + 3")
        expected = [IntegerToken(-5), Token.PLUS, IntegerToken(3)]
        self.assertEqual(result, expected)
    ...
    def test_tokenize_negative_float_zero(self):
        """Test tokenizing negative float zero."""
        result = tokenize("-0.0")
        self.assertEqual(result, [FloatToken(-0.0)])
    
    def test_tokenize_negative_float_with_trailing_zeros(self):
        """Test tokenizing negative floats with trailing zeros."""
        result = tokenize("-1.500")
        self.assertEqual(result, [FloatToken(-1.5)])
    
    def test_tokenize_negative_float_in_expression(self):
        """Test tokenizing negative floats in expressions."""
        result = tokenize("let pi float = -3.14")
        expected = [Token.LET, IdentifierToken("pi"), Token.FLOAT_TYPE, Token.EQUALS, FloatToken(-3.14)]
        self.assertEqual(result, expected)
    
    def test_tokenize_negative_float_arithmetic(self):
        """Test tokenizing negative floats in arithmetic expressions."""
        result = tokenize("-2.5 * 4.0")
        expected = [FloatToken(-2.5), Token.MULTIPLY, FloatToken(4.0)]
        self.assertEqual(result, expected)
    
    def test_tokenize_mixed_negative_positive(self):
        """Test tokenizing expressions with both negative and positive numbers."""
        result = tokenize("-5 + 10 - 3")
        expected = [IntegerToken(-5), Token.PLUS, IntegerToken(10), Token.MINUS, IntegerToken(3)]
        self.assertEqual(result, expected)
