...
class TestParser(unittest.TestCase):
    ...
    def _parse_expression(self, code: str) -> List[Statement]:
    ...
    def test_parse_parentheses_simple(self):
        expected = [self._let_stmt("result", Type.INTEGER, self._int_lit(5))]
    ...
    def test_parse_print_statement_integer(self):
        expected = [self._print_stmt(self._int_lit(42))]
        self._assert_parse_equals("print 42", expected)
    
    def test_parse_print_statement_expression(self):
        expected = [self._print_stmt(self._binary_expr(self._int_lit(10), BinaryOperator.MULTIPLICATION, self._int_lit(5)))]
    ...
    def test_parse_statement_separators_only(self):
        ...
        tokens = [Token.STATEMENT_SEPARATOR, Token.STATEMENT_SEPARATOR]
        ast = parse(tokens)
        self.assertEqual(ast, [])
    
    def test_parse_trailing_statement_separator(self):
        # Using tokens directly for this edge case  
        tokens = [Token.LET, IdentifierToken("x"), Token.INTEGER_TYPE, Token.EQUALS, IntegerToken(1), Token.STATEMENT_SEPARATOR]
        ast = parse(tokens)
        expected = [Let("x", Type.INTEGER, IntegerLiteral(1))]
    ...
    def test_parse_boolean_from_string(self):
        expected = [self._let_stmt("isTrue", Type.BOOLEAN, self._bool_lit(True))]
    ...
    def test_parse_simple_if_statement(self):
        expected = [self._if_stmt(self._bool_lit(True), [self._print_stmt(self._int_lit(5))])]
        self._assert_parse_equals("if true\n    print 5", expected)
    
    def test_parse_if_with_expression(self):
        expected = [self._if_stmt(self._binary_expr(self._var("x"), BinaryOperator.GREATER_THAN, self._int_lit(5)), 
                                 [self._print_stmt(self._var("x"))])]
        self._assert_parse_equals("if x > 5\n    print x", expected)
    
    def test_parse_if_with_multiple_statements(self):
        expected = [self._if_stmt(self._bool_lit(True), 
                                 [self._let_stmt("x", Type.INTEGER, self._int_lit(5)), 
                                  self._print_stmt(self._var("x"))])]
        self._assert_parse_equals("if true\n    let x integer = 5\n    print x", expected)
    
    def test_parse_if_followed_by_regular_statement(self):
        expected = [self._if_stmt(self._bool_lit(True), [self._print_stmt(self._int_lit(1))]), 
    ...
    def test_parse_error_if_missing_condition(self):
        tokens = [Token.IF, Token.STATEMENT_SEPARATOR]
        with self.assertRaises(ParseError) as cm:
            parse(tokens)
        self.assertEqual(str(cm.exception), "Expected integer, float, identifier, boolean, or opening parenthesis")
    
    def test_parse_error_if_missing_body(self):
        tokens = [Token.IF, Token.TRUE, Token.STATEMENT_SEPARATOR]
        with self.assertRaises(ParseError) as cm:
            parse(tokens)
    ...
    def test_parse_simple_set_statement(self):
        expected = [self._set_stmt("x", self._int_lit(42))]
        self._assert_parse_equals("set x = 42", expected)
    
    def test_parse_set_with_expression(self):
        expected = [self._set_stmt("x", self._binary_expr(self._var("y"), BinaryOperator.ADDITION, self._int_lit(5)))]
        self._assert_parse_equals("set x = y + 5", expected)
    
    def test_parse_set_from_string(self):
        expected = [self._set_stmt("counter", self._binary_expr(self._var("counter"), BinaryOperator.ADDITION, self._int_lit(1)))]
        self._assert_parse_equals("set counter = counter + 1", expected)
    
    # While loop tests
    def test_parse_simple_while_statement(self):
        expected = [self._while_stmt(self._bool_lit(True), [self._print_stmt(self._int_lit(5))])]
        self._assert_parse_equals("while true\n    print 5", expected)
    
    def test_parse_while_with_condition_expression(self):
        expected = [self._while_stmt(self._binary_expr(self._var("x"), BinaryOperator.GREATER_THAN, self._int_lit(0)), 
                                    [self._print_stmt(self._var("x"))])]
        self._assert_parse_equals("while x > 0\n    print x", expected)
    
    def test_parse_while_with_multiple_statements(self):
        expected = [self._while_stmt(self._bool_lit(True), 
                                    [self._print_stmt(self._var("x")), 
                                     self._set_stmt("x", self._int_lit(5))])]
        self._assert_parse_equals("while true\n    print x\n    set x = 5", expected)
    
    def test_parse_while_followed_by_regular_statement(self):
        expected = [self._while_stmt(self._bool_lit(False), [self._print_stmt(self._int_lit(1))]), 
                   self._print_stmt(self._int_lit(2))]
    ...
    def test_parse_error_while_missing_condition(self):
        tokens = [Token.WHILE, Token.STATEMENT_SEPARATOR]
        with self.assertRaises(ParseError) as cm:
            parse(tokens)
        self.assertEqual(str(cm.exception), "Expected integer, float, identifier, boolean, or opening parenthesis")
    
    def test_parse_error_while_missing_body(self):
        tokens = [Token.WHILE, Token.TRUE, Token.STATEMENT_SEPARATOR]
        with self.assertRaises(ParseError) as cm:
            parse(tokens)
    ...
    def test_parse_modulus_operation(self):
        expected = [self._let_stmt("remainder", Type.INTEGER, 
    ...
    def test_parse_simple_float(self):
        expected = [self._let_stmt("pi", Type.FLOAT, self._float_lit(3.14))]
        self._assert_parse_equals("let pi float = 3.14", expected)
    
    def test_parse_float_arithmetic(self):
        expected = [self._let_stmt("result", Type.FLOAT, 
    ...
    def test_parse_float_from_string(self):
        expected = [self._let_stmt("value", Type.FLOAT, self._float_lit(3.14))]
    ...
    def test_parse_function_call_expression(self):
        ...
        expected = [self._print_stmt(FunctionCall("add", [self._int_lit(5), self._int_lit(10)]))]
    ...
    def test_parse_function_call_in_let(self):
        ...
        expected = [self._let_stmt("result", Type.INTEGER, FunctionCall("add", [self._int_lit(3), self._int_lit(7)]))]
    ...
    def test_parse_list_type_declaration(self):
        ...
        expected = [Let("nums", ListType(Type.INTEGER), ListLiteral([self._int_lit(1), self._int_lit(2), self._int_lit(3)]))]
    ...
    def test_parse_empty_list_literal(self):
        ...
        expected = [Let("empty", ListType(Type.INTEGER), ListLiteral([]))]
    ...
    def test_parse_list_with_boolean_elements(self):
        ...
        expected = [Let("flags", ListType(Type.BOOLEAN), ListLiteral([self._bool_lit(True), self._bool_lit(False)]))]
    ...
    def test_parse_list_with_float_elements(self):
        ...
        expected = [Let("values", ListType(Type.FLOAT), ListLiteral([self._float_lit(1.5), self._float_lit(2.0)]))]
    ...
    def test_parse_repeat_function_call(self):
        ...
        expected = [Let("zeros", ListType(Type.INTEGER), RepeatCall(self._int_lit(0), self._int_lit(5)))]
    ...
    def test_parse_len_function_call(self):
        ...
        expected = [self._print_stmt(LenCall(Variable("nums")))]
        self._assert_parse_equals("print len(nums)", expected)
    
    def test_parse_list_access(self):
        """Test parsing list access."""
        expected = [self._print_stmt(ListAccess(Variable("nums"), self._int_lit(0)))]
    ...
    def test_parse_list_access_with_expression(self):
        ...
        expected = [self._print_stmt(ListAccess(Variable("nums"), BinaryExpression(Variable("i"), BinaryOperator.ADDITION, self._int_lit(1))))]
        self._assert_parse_equals("print nums[i + 1]", expected)
    
    def test_parse_list_assignment(self):
        """Test parsing list assignment."""
        expected = [ListAssignment("nums", self._int_lit(1), self._int_lit(42))]
    ...
    def test_parse_list_assignment_with_expression(self):
        ...
        expected = [ListAssignment("nums", Variable("i"), BinaryExpression(Variable("x"), BinaryOperator.MULTIPLICATION, self._int_lit(2)))]
    ...
    def test_parse_list_with_variable_elements(self):
        ...
        expected = [Let("numbers", ListType(Type.INTEGER), ListLiteral([Variable("x"), Variable("y"), self._int_lit(3)]))]
    ...
    def test_parse_list_with_expression_elements(self):
        ...
        expected = [Let("computed", ListType(Type.INTEGER), ListLiteral([
    ...
    def test_parse_nested_list_access(self):
        ...
        expected = [self._print_stmt(RepeatCall(ListAccess(Variable("nums"), self._int_lit(0)), LenCall(Variable("other"))))]
    ...
    def _float_lit(self, value):
        """Helper to create FloatLiteral."""
        return FloatLiteral(value)
    ...
    def test_parse_negative_zero(self):
        ...
        expected = [self._let_stmt("x", Type.INTEGER, self._int_lit(0))]
    ...
    def test_parse_negative_float_zero(self):
        ...
        expected = [self._let_stmt("x", Type.FLOAT, self._float_lit(-0.0))]
